...
3. **Implement `TreeSelectionModel<T>` and `TreeExpansionModel<T>`**
...
1. **Create `TreeViewport` (Canvas Region)**
...
2. **Implement Flattened Row Cache (`FlattenedTree<T>`)**
...
3. **Define Render Context & Passes**
...
3. **Implement `TreeView` Top-Level Component**
...
2. **Implement Content Factory & Adapter**
...
1. **Implement Drag and Drop (`TreeDragDropController`)**
   - Internal reordering with visual drop hints (overlay canvas).
   - External payload support.
2. **Implement In-place Editing (`TreeEditController`)**
